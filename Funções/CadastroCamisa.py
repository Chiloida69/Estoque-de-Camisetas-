...
def CadastroCamisa(Marca , time): #Define a função para cadastro das camisas.
    ...
    while True:
        Modelo = input(f'Cadastre o modelo da camisa (ou digite "sair" para terminar): ')
        # Condição para sair do loop
        if Modelo.lower() == 'sair':
            break #Sai do loop se o usuário digitar 'sair'.
        # Solicita os dados da camisa
        Tamanho = input(f'Cadastre o tamanho da camisa (P, M, G, GG) para o modelo {time}, {Modelo}: ')
        Cor = input(f'Cadastre a cor da camisa para o modelo {time},{Modelo}: ')
        try:
            Codigo = int(input(f'Cadastre um código para a camisa {time},{Modelo}: '))
            Quantidade = int(input(f'Cadastre a quantidade de camisas para o modelo {time},{Modelo}: '))
        except ValueError:
